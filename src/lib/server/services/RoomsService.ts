import type { CreateRoomDto, JoinRoomDto } from '$lib/server/dtos';
import { RoomNotFoundError, UserNotFoundError, VoteNotFoundError } from '$lib/server/errors';
import { RoomRepository, UserRepository, VoteRepository } from '$lib/server/repositories';
import type { ClientRoom, ClientUser, ClientVote, Room, User, Vote, VoteCard } from '$lib/types';
import type { RoundHistory } from '$lib/types/EventData';
import { ClientUserSchema } from '../dtos/ClientUserSchema';
import type { UpdateRoomDto } from '../dtos/UpdateRoomDtoSchema';
...
class RoomsService {
	...
	/* for finished round */
	async getVotes(roomId: Room['id'], round: Room['round']): Promise<ClientVote[]> {
		const votes = await VoteRepository.getAllRoundVotes(roomId, round);
		return votes;
	}
	...
	async goToNextRound(slug: Room['slug']): Promise<{ room: ClientRoom; prevRound: RoundHistory }> {
	...
	async addVote(room: Room, userId: User['id'], value: VoteCard): Promise<Vote> {
		const existingVote = await VoteRepository.getOne(room.id, room.round, userId);
		if (existingVote) {
			const vote = await VoteRepository.update(existingVote.id, value, room.round);
			if (!vote) {
				throw new VoteNotFoundError();
			}
			return vote;
		}
		const vote = await VoteRepository.create(room.id, userId, value, room.round);
		if (!vote) {
			throw new VoteNotFoundError();
		}
		return vote;
	...
	async update(slug: Room['slug'], changes: UpdateRoomDto): Promise<ClientRoom> {
