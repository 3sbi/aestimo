...
import { usersTable, votesTable } from '$lib/server/db/schemas';
import type { ClientVote, Room, User, Vote, VoteCard } from '$lib/types';
import { and, eq, sql } from 'drizzle-orm';

class VoteRepository {
	static async getOne(roomId: Room['id'], round: Room['round'], userId: User['id']) {
		const vote = await db
			.select()
			.from(votesTable)
			.where(
	...
	static async create(
		...
		const vote = await db.insert(votesTable).values({ roomId, userId, value, round }).returning();
		return vote.pop();
	}
