...
import { roomsTable } from '$lib/server/db/schemas';
import type { CreateRoomDto } from '$lib/server/dtos';
...
class RoomRepository {
	...
	static async delete(slug: Room['slug']) {
		const result = await db.delete(roomsTable).where(eq(roomsTable.slug, slug)).returning();
		const room = result.pop();
		return !!room;
	}
}
